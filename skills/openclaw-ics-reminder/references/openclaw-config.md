...
- The entry key is `ics-reminder` because the skill uses `metadata.openclaw.skillKey`.
- `apiKey` maps to `REMINDER_API_TOKEN` because the skill declares `metadata.openclaw.primaryEnv`.
